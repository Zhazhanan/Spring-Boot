...
public class ElasticsearchUtils {
    ...
    public boolean isIndexExist(String index) {
        ...
        if (inExistsResponse.isExists()) {
            LOGGER.warn("Index [" + index + "] is exist!");
        }
...
//                        if (0< ss[0].indexOf("|")) {
//                            String[] keyanme = ss[0].split("\\|");
//                            for (String s1 : keyanme) {
//                                boolQuery.should(QueryBuilders.wildcardQuery(s1, "*"+ss[1]+"*"));
//                            }
//                        }else{
//                            boolQuery.must(QueryBuilders.matchQuery(s.split("=")[0], s.split("=")[1]));
//                        }
                        if (-1 != ss[0].indexOf("|")) {
                            MultiMatchQueryBuilder matchQueryBuilder = QueryBuilders.multiMatchQuery(s.split("=")[1], ss[0].split("\\|")[0], ss[0].split("\\|")[1]);
                            matchQueryBuilder.minimumShouldMatch("2").prefixLength(2).fuzziness(Fuzziness.TWO);
                            boolQuery.must(matchQueryBuilder);
                        } else {
                            boolQuery.must(QueryBuilders.matchQuery(s.split("=")[0], s.split("=")[1]).minimumShouldMatch("2").prefixLength(2).fuzziness(Fuzziness.TWO));
                        }
                    }
                }
            ...
            for (String s : matchStr.split(",")) {
                ...
                if (matchPhrase == Boolean.TRUE) {
                    ...
                    if (-1 != ss[0].indexOf("|")) {
                        ...
                        boolQuery.must(QueryBuilders.multiMatchQuery(s.split("=")[1], ss[0].split("\\|")[0], ss[0].split("\\|")[1]));
                    } else {
                        boolQuery.must(QueryBuilders.matchQuery(s.split("=")[0], s.split("=")[1]));
                    }
                }
